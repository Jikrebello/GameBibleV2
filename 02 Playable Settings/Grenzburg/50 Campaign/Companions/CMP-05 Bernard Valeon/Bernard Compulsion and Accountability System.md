...
One authored incident, Act II–III window, and the single exception to the pool's protective rule: **the owner is somebody he loves.**

The object is **Hannel Klee's** — small, hers, and loved. (Her mother's thimble, or the bone button-hook she mends with; execution picks one and keeps it.) The girl who was once blamed for him loses something to him for real.

- Standard tell sequence applies. The urge is legible; the intervention window exists.
- **Earned trust genuinely prevents it.** If `bernard_self_report_state` is `reliable`, the incident does not fire at all. It is the only incident in the pool that retroactively rewards the accountability play, and the player never learns they avoided it.
- If completed, the return circuit is unlike any other. Hannel's response is not gracious and not a boundary statement: she has to decide whether to do to him what was done to her — report it, let it be public, let it cost him — or protect him the way nobody protected her. She takes a full day to answer.

Either way, some version of: *“Now it's real.”*

- Repair follows the ordinary loop, but her relationship state caps at `repaired-guarded` and never returns to before. Bernard's *permanent* loss lives at the finale; hers is lasting strain, not severance.
- Authored, one-shot, non-farmable, and never replaced by another incident.

### The Salve, and Why It Is Not In This Pool

The Act III finale's hidden portion in [[The Open Hospice]] is **not** a systemic incident and must not be generated by this system.

It is authored, it is for a named child Bernard loves, it is taken before the count begins, and its live choice is disclosure rather than acquisition. The pool's laws — no quest-critical items, no supplies required to keep a critical-path target alive — would forbid it, correctly, as a random event. It exists once, on purpose, written by hand.
