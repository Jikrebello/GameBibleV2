---
type: quest
status: canon
canon_level: local
visibility: author-only
era: ["Present Day"]
region: ["Grenzburg", "Old Market", "Lantern Ward"]
culture: ["Vortharan", "Sestaran", "Eclessian"]
related_factions: ["Blackjack Corps"]
appears_in: ["Grenzburg"]
needs: []
tags: [narrative-rewrite]
quest_class: companion
act: act-1
availability_window: ["Open Autumn", "Early Winter"]
repeatability: one-shot
cutoff_result: remains-available
world_state_outputs: ["tavio_friendship_episode", "yard_fair_state", "tavio_prize_state", "tavio_approval", "tavio_romance_interest_signaled"]
reward_unlocks: ["Declared Handicap", "Tavio Friendship Scene"]
required_locations: ["Old Market", "Bridgehold", "Blackjack Yard"]
blocking_status: blocked
architecture_status: migrated
companion_content_role: friendship
companion_campaign_stage: friendship
quest_owner: "Tavio Meran"
quest_sequence: 4
companion_id: "CMP-07"
faction_line: ""
faction_branch: ""
chapter_id: ""
main_sequence: 0
story_gate: relationship
estimated_hours: 0.5
gameplay_pillars: [social, minigames, traversal, character]
required_states: ["tavio_act1_chapter_resolved", "tavio_approval_open"]
optional_state_inputs: ["yard_refugee_state", "hammer_ward_services", "old_market_state"]
field_experience_gate: []
companion_required: "Tavio Meran"
duo_quest: false
pause_points: []
relationship_gate: "Universal friendship episode"
relationship_variant: ""
preparation_inputs: []
preparation_threshold: ""
autonomous_deadline: ""
autonomous_resolution: "See Companion Autonomy and Act Convergence; untracked content uses stable-colleague treatment."
changed_reentry: ""
ensemble_inputs: []
identity_hooks: ["Sestaran", "Peasant/Labour", "Merchant/Craft", "Drifter/Vagabond"]
grounded_rewrite_status: approved
external_stakes: ["Blackjack fair", "honest contests", "refugee and forge-family meal"]
banter_priority: optional
player_interest_gate: none
missed_arc_treatment: stable-colleague
encounter_deck: []
persistence_scope: personal
postgame_season: ""
emotional_rewrite_status: cleared
dialogue_status: approved
remote_claim_status: compliant
archetype_pressure: "Competition becomes worth: test whether Tavio can stop, lose, or need help without surrendering athletic joy."
emotional_function: "Keep fair games, absurd prizes, food, and delight without villainy"
pleasure_beat: "Competition, crowds, food, physical effort, ridiculous prizes, and celebrating improvement."
care_beat: "Tavio trains beside people, celebrates their effort, and calls a halt before glory spends their bodies."
player_archetype_hooks: []
---
# Games Without a Crown

## Quest Function

This universal friendship episode gives Tavio a complete, uninterrupted experience of competition without title, fraud, villain, or crisis.

It is one-on-one in presentation, but not a combat duo quest.

## Invitation

Tavio asks the player to help run a small fair for debtors, forge families, and displaced households. He has no patron, no championship purse, and no authority beyond what participants freely grant.

The player chooses three activities:

- bridge-ring toss;
- marked-barrel relay;
- safe spear-vault demonstration;
- blind spice tasting;
- cookware percussion;
- absurd-prize judging;
- children's chalk arena;
- pack-and-carry team race.

## Rules

Every game has:

- a declared measure;
- an honest handicap;
- a participant-controlled stop;
- a non-humiliating way to withdraw;
- a prize whose joke never depends on poverty, injury, culture, or debt.

The player may compete, officiate, cook, keep score, or invent one final prize.

A female player may also choose one plainly framed private-interest line after the fair. Selecting it writes `tavio_romance_interest_signaled`; declining or ignoring it leaves the friendship untouched. Tavio's earlier praise, physical ease, and game invitations are never retroactively treated as flirting.

## Steffen

One competitor is nineteen, from the crane yards, and better than he has any business being.

He wins the safe spear-vault demonstration. When Tavio praises him — publicly, precisely, naming the thing he did well — the boy lights up like a lamp and is unbearable about it for the rest of the afternoon, and everybody forgives him because he is nineteen and it is a fair.

He gives his name as **Steffen**. Nothing else about him surfaces here: no pits, no gear, no shadow. The player should simply like him. ([[Steffen Krall]].)

## The Day

The episode moves through Old Market, Bridgehold, and the Blackjack Yard. Small choices alter dialogue and ambient fair scenes:

- let a child change a rule that excludes younger players;
- catch Tavio quietly correcting a score against himself;
- decide whether a champion should compete under a handicap or serve as teacher;
- choose a winner, shared victory, or comic award;
- accept a loss without demanding a rematch.

There is no attack, theft, sabotage, hidden moral test, or interrupted climax.

## Closing Scene

After cleanup, Tavio admits this is the first crowd since Vorthara whose enjoyment did not depend on him being unbeatable.

The player may answer as:

- a competitor;
- an honest second;
- a friend who does not need a performance;
- someone who simply enjoyed the day.

Winning, losing, or tying changes dialogue only.

## Rewards

- `tavio_friendship_episode`;
- **Declared Handicap**, a small social advantage when proposing fair contests;
- an absurd keepsake chosen during the fair;
- friendship scene access independent of romance or bond.

## Navigation

- [[Tavio Companion Campaign]]
- [[Tavio Field Familiarity and Relationships]]
- [[Proof Under Load]]
- [[The Challenger's Road]]
- [[Blackjack Yard]]
