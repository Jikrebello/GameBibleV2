...
**Greed's network. Masks: Avarix** (Ishkatar's serpent-cult, per the register); its own initiates say **the Principal.**
...
**Tellers** (embedded clerks), **Factors-Below** (cell masters running captured desks), **the Comptroller's Circle** (masters; their initiation is rumored to require depositing something irreplaceable and never withdrawing it). Signs: institutions whose bad loans cluster on the vulnerable with actuarial precision; hoards discovered at deaths that beggar the estate's known income; account-books that balance perfectly and smell faintly of the reptile house.

## Relations

The Medori hunt them (nothing threatens a real bank like a fake one inside it); the Creditor regards them the way an ocean regards a leak — same substance, embarrassing scale; the Vault of Names has burned two Under-Ledger cells that tried collateralizing true names without a license, which tells you what the Covenant considers the actual crime.
