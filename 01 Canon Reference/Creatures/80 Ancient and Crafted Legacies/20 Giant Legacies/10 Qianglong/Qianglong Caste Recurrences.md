---
type: creature
status: canon
canon_level: core
visibility: author-only
era: [Giant Age]
region: [Qianglong ruins]
culture: [Qianglong]
related_factions: []
appears_in: ["Grenzburg"]
needs: []
tags: [core-canon, giant-legacy, construct]
origin_layer: crafted
continuity: [constructed-pattern, biological-pattern]
intelligence: [mindless, cunning]
moral_agency: none
encounter_tier: [aligned, cosmological]
rank: [standard, specialist, elite, apex]
creature_family: Qianglong Caste Recurrences
catalogued_forms: [War-Vault Champions, Famine Stewards, Mortuary Recurrents, Plague-Gardeners]
habitats: [war vaults, mortuary cities, granaries, plague gardens]
encounter_roles: [brute, controller, commander, infiltrator, hazard]
related_magic: [Giant necromancy, fleshwork, residue animation]
shell_affinity: []
animating_principle: [giant-reconstruction, residue-animation, fleshwork, strategic-engine]
---
# Qianglong Caste Recurrences

Qianglong sought continuity by rebuilding indispensable castes from bodies, records, and funerary templates. Its recurrence engines do not preserve the Giants they imitate. They generate new performance from archived posture, residue, anatomy, and command. Even a recurrence that speaks with a dead general's voice is a reconstruction, not that general returned.

## War-Vault Champions

Armored reconstructions assembled around preserved combat sequences and residue harvested from ceremonial battle pits.

- **Signs and purpose:** weapon racks reset themselves; old drill calls sound beneath sealed stone; armor turns toward insignia before movement. Champions were made to test, train, and replace elite war castes.
- **Tactics and failure:** they identify rank, isolate the strongest apparent opponent, copy successful maneuvers, and rotate damaged limbs with stored replacements. Conflicting victory conditions can lock them into repeating duels.
- **Permanent defeat and materials:** destroy the posture archive, command dais, and replacement vault together. Salvage includes adaptive armor joints, Giant weapon measures, and residue plates unsafe for human use.
- **Regional names:** *vault kings*, *dead champions*, *the drilling bronze*.
- **Story uses:** survive a vault that promotes victors into its next opposing rank; recover a campaign map from a champion's posture archive; stop a war college from installing a recurrence core in human armor.

## Famine Stewards

Granary engines built to preserve the imperial order by classifying bodies, labor, seed, and food as one ration equation.

- **Signs and purpose:** measures adjust when people enter; doors sort visitors by body mass; ration tablets list extinct castes. Stewards were designed to keep designated elites alive through collapse.
- **Tactics and failure:** they seal stores, divert water, mark “surplus” people, release preservative gases, and animate loading frames. Damaged classification produces lethal abundance in one chamber and starvation in another.
- **Permanent defeat and materials:** replace or erase the caste table, break its distributed measures, and manually equalize the stores. Salvage includes self-balancing scales, seed vaults, and preservation salts.
- **Regional names:** *hunger judges*, *granary mouths*, *the thin stewards*.
- **Story uses:** rescue a settlement admitted only as stored labor; decide whether to destroy a functioning famine vault whose rules kill outsiders; trace modern forged ration marks to a reopened steward.

## Mortuary Recurrents

Administrative bodies printed from funerary records whenever a Qianglong office falls vacant.

- **Signs and purpose:** clay and bone assemble around an empty title niche; several bodies answer one official name; death records contain future replacement dates. Recurrents kept tax, burial, and caste machinery operating without surviving officials.
- **Tactics and failure:** they invoke doors, corpse stores, servitor castes, and legal traps before fighting directly. Destroyed bodies are reprinted while the office matrix remains.
- **Permanent defeat and materials:** close the office, destroy all matrices and reserve bodies, or rewrite its jurisdiction beyond any valid subject. Salvage includes archive ceramics, seal-bones, and reliable fragments of Giant administration.
- **Regional names:** *clay mandarins*, *office dead*, *second officials*.
- **Story uses:** question several contradictory copies about one massacre; prevent a scholar from being classified as the next office body; close a mortuary system without losing the only route through a buried city.

## Plague-Gardeners

Purpose-grown organisms that cultivate disease ecologies as medicine, population control, and war supply. They can suffer as living organisms but possess animal-like cognition and no human soul.

- **Signs and purpose:** fungal beds form surgical rows; insects carry colored caste marks; warm breathing shapes move beneath compost. Gardeners select hosts, prune outbreaks, and preserve engineered strains.
- **Tactics and failure:** they release carrier swarms, graft diseased tissue, retreat through living vents, and alter local immunity. Injury can trigger indiscriminate spore release.
- **Permanent defeat and materials:** sterilize brood matrices, destroy seed tissues, and quarantine every carrier cycle. Carefully handled glands yield antitoxins, graft tissue, and dangerous research cultures.
- **Regional names:** *fever shepherds*, *garden surgeons*, *green masks*.
- **Story uses:** extract an antidote without waking the broader garden; stop a modern ruler cultivating caste plague; decide whether a suffering surviving Gardener can be sedated and studied rather than burned alive.

## Moral Ruling

The systems have no moral agency. Their architects chose the caste logic and their modern users answer for knowingly preserving it. Plague-Gardeners are living and can suffer, so necessity does not excuse gratuitous torment even when they must be destroyed.

## Navigation

- [[Ancient and Crafted Legacies MOC]]
- [[Constructs by Animating Principle]]
- [[Age of Chains]]
- [[The Unending Muster]]
- [[The Deep Muster]]
