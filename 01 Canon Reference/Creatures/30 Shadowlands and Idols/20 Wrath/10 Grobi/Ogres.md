---
type: creature
status: canon
canon_level: local
visibility: public
era: []
region: []
culture: []
related_factions: []
appears_in: []
needs: []
tags: []
origin_layer: shadowlands
continuity: wrath-rewritten-life
intelligence: cunning
moral_agency: limited
encounter_tier: aligned
rank: [apex]
creature_family: Grobi
catalogued_forms: [Ogres]
habitats: [southern cold belt, taiga, permafrost, migration corridors]
encounter_roles: [raider, skirmisher, siege, predator]
related_magic: [Idol influence]
idol_affinity: wrath
manifestation_class: []
---
# Ogres

Ogres are the fourth Grobi stage: bodies built for siege, hauling, and survival through injuries that would stop smaller stages. Their scale imposes an enormous food demand and narrows attention toward immediate bodily needs, but does not erase memory, cunning, recognition, or all moral agency.

## Ecology and Lifecycle

Most ogres grow from orcs at the center of exceptionally successful raiding economies. Few warbands can sustain more than one for long. Starvation strips fat, slows regeneration, and may force an acquired ogre back toward orc scale. A catastrophic injury or further surplus can instead begin the unstable fifth-stage crisis that usually kills the body and rarely produces a [[Trolls|troll]].

## Warband Role

Ogres break gates, move timber and stone, carry winter stores, anchor shield lines, and embody a leader's power to feed dependents. Smaller Grobi usually choose routes, targets, and terms. An ogre that retains strong orc memory may lead directly, though the metabolic burden makes sustained administration difficult.

## Signs

- trees torn up rather than cut for fire or siege work;
- deep prints beside drag furrows and crushed sled runners;
- stores separated into dense fat, marrow, and preserved organs;
- fortifications tested by repeated impacts at shoulder height;
- smaller Grobi abandoning a route that cannot feed the heavy stage.

## Threat Pattern

Ogres use mass, reach, thrown debris, and whatever the warband places in their hands. They are vulnerable to heat stress, difficult terrain, isolation from food, and tactics that force repeated changes of direction. Their regeneration consumes reserves quickly; a long fight can be safer than a short attempt to trade blows.

## Society and Agency

Warbands may treat ogres as kin, symbols, weapons, or captives. An ogre can obey, bargain, remember insult, protect a dependent, or choose predation. Hunger constrains those choices without making them meaningless.

## Weakness and Permanent Defeat

Difficult ground, heat, isolation, starvation, and repeated forced movement exhaust an ogre faster than direct injury. Its regenerative mass can close terrible wounds but cannot replace energy it does not possess. Destroying the fourth-stage gland and separating major tissue masses ends the individual pattern; leaving a fed carcass intact risks a violent final recovery.

## Materials and Remains

Ogre tendon, hide, dense bone, and fourth-stage gland tissue have siege, surgical, and alchemical value. The gland is dangerously close to the instability that produces trolls. Frontier law normally requires witnessed destruction unless a licensed healer or investigator takes custody.

## Regional Names

Folk settlements use **gate-Grobi**; Ättar warbands say **heavy-stage**; Eclessian soldiers use **ogre** and **wall-breaker** interchangeably.

## Navigation

- [[Bestiary MOC]]
- [[Grobi Overview]]
