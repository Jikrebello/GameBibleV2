...
appears_in: []
needs:
  - review all seeds against regional culture dossiers before promotion
  - retain no more than one true Idol Regnant succession from each block
  - reconcile any retained ancient title with the Named Figures register
tags:
  - research
  - historical-seeds
  - whole-history-matrix
  - mythic-history
---
...
Each historical block is planted through four independent lanes:

1. **Human hero and company:** ordinary people whose local competence may grow into public history.
2. **Land, folklore, and local encounter:** dangerous hospitality, place-law, monsters, journeys, and stories explaining customs or landscapes.
3. **Institution, memory, and relic:** songs, offices, legal phrases, ritual objects, archives, titles, and practices that outlive their makers.
4. **Idol, Menhir, and other metaphysical pressure:** cultural manifestations, feeding architecture, Mask rivalry, Counter-Countenance traces, and very rare Regnant-succession candidates.

The lanes may cross, but they must not collapse into a single master plot. A company can encounter one local Mask without secretly causing an Idol succession; a folk story can preserve a Menhir bargain without establishing the authorial history of creation; a surviving relic can be genuine without authenticating every later claim made about it.

### Continuity Shorthand

- `human-soul`: a living or routed human person made in the Image.
- `created-spirit`: a true angelic person; not a human soul.
- `yolk-sovereignty`: the continuity of a Menhir or Idol spread through sovereign, law, realm, cohort remnants, and manifestations.
- `court-law`: a Faewild court person, place-mind, or law-organism as controlled by its own dossier.
- `idol-pattern`: fauna or a dependent Mask projected through a feeding architecture.
- `shadow-pattern`: wild Shadowlands life not necessarily grown by one Idol.
- `infrastructure-office`: an office, title, route, or magical institution; never automatically the bearer.
- `residue`: memory and emotional persistence after a human soul has routed.
- `bound-human-soul`: a human soul unlawfully retained in an object, office, corpse, or construct.
- `titan-continuity`: the uninterrupted, potentially distributed continuity of a direct Titan-Firstborn. Dismemberment does not by itself establish death, multiplication, or the presence of a human soul.
- `giant-continuity`: the accountable nonhuman personal continuity of a High or Dynastic Giant, including a genuine uninterrupted Nythalum Continuant. It is not a human soul-route.
- **Giant Reconstruction:** a Giant caste, ruler, memory, or administrative performance rebuilt after interruption from bodies, records, residue, or copied patterns. Its continuity is `constructed-pattern`, not a separate Giant category; however persuasive its self-claim, it is not the historical Giant.
- **Copy-claim categories:** any alleged returning ancestor must resolve separately as `constructed-pattern`, `infrastructure-office`, `residue`, `legend-persona`, `idol-pattern`, or genuine `titan-continuity` / `giant-continuity`. “Copy” is an audit problem, not a continuity class of its own.
- `legend-persona`: a historically accreted role or performed biography that may answer under a person's name without containing or becoming that person's soul.
- `constructed-pattern` and `biological-pattern`: other nonhuman continuities used only where crafted systems or living creatures are actually implicated.

### Compact Card-Field Equivalence

These early-age cards use compact labels while retaining the full [[Historical Seed Card Template]] interface:

- **Era and scope** carries era, region, culture, lane, scale, and principal carrier where these can responsibly be fixed.
- **Humble ground**, **ordinary origin**, **ordinary event**, or **ordinary problem** carries the humble origin. On a nonhuman metaphysical card, **seed function**, **seed account**, **candidate account**, **probe account**, **campaign account**, **coup account**, or **cycle account** instead records the first bounded anchor; where no mortal origin is permitted, the card states that absence.
- **Scriptural lens**, **comparative grammar**, historical pressure, and **World Egg torque** are the transformation formula.
- **Continuities** classifies the human soul, legend, residue, office, Mask, Patron, Menhir, and object relations.
- **Present-day traces** carries the 1360 AR residue and marks claimed traces as claims rather than authorial fact.
...
### Human-Coherence Control

[[Human Coherence and Discoverability Pass]] governs the ordinary-language reading of every card. Terms such as *recognizes*, *answers*, *feeds*, *gains mass*, *stabilizes*, *aligns*, *refuses*, and *opens a route* are never complete explanations. A developed version must state the prior condition, chosen act, exact supernatural trigger where relevant, observable result, direct cost, later consequence, surviving evidence, and invalid larger inference. Deliberately open facts remain open; an unspecified mechanism does not count as mystery.

---

# Block I — Creation through the Age of Chains

## Block-I Density and Protected Empty Beds

| Period | Human/company | Land/folklore | Institution/memory | Metaphysical pressure |
|---|---:|---:|---:|---:|
| Creation and humanity's first condition | deliberately empty | 1 very light etiological lead | deliberately empty | no mortal witness permitted |
| Great Rebellion and Yolk Exiles | deliberately empty | deliberately empty | later apocrypha only | 1 formation seed, not a succession |
| Faewild Schism | no mortal participants | 1 distributed scar cluster | contradictory later memories only | enduring Menhir pressure without a new sovereign |
| Deep Giant time | 2 | 2 | 2 | 2 |
| Late Chains | 2 | 1 | 1 | 1 plus one Counter-Countenance trace |

Protected empty beds:

- Do not name a first human couple, define the exact mechanics of humanity's fall, or create an eyewitness company for Creation.
- Do not turn the Great Rebellion into a Greek-, Norse-, or Hindu-style war among peer gods. The Lord is uncreated; every rebel and loyal host is created.
- Do not make the Menhir necessary creators of natural law. Their rebellion parasitically fused them to laws the Lord had already created.
- Do not make human dreaming or vice create the Shadowlands or the Idols.
- Do not explain [[The Weird of History — The Unexplained Register|the Glass]], the bird-marked sites, Giant lunar ascent, or any other registered Weird.
- Do not manifest a saint before the Redeemer's work. Ancient righteous humans may receive aid, warning, or a bounded earthly commission without becoming pre-Redeemer saintly manifestations or collapsing the two dispensations.

Regional Giant-sphere coverage is deliberately local rather than a second liberation epic:

| Giant sphere | Local human or folk carrier |
|---|---|
| Anshara | I-H01, the palace refuge of the People Beneath the Bowl |
| Qianglong | I-H02, the Midwives of the Unnumbered working against caste measures |
| Ishkatar | I-H03, the Funeral Thieves at a corpse-reclamation quay |
| Xolotl | I-L03, the child who disrupts one estate's blood-astronomical noon |
| Nythalum | I-L04, the work-camp thief who breaks one royal labor calendar |

None of these figures causes an empire's fall or substitutes for the Great Prophet.

## Late-Chains–Exodus Bridge Crosswalk

> [!important] Planning interface, not a new chronology
> This crosswalk applies the controlling history in [[The Great Unbinding]] through the fuller research interface in [[The Great Unbinding - Collapse, Remnant, and the Glass Ark]]. It does not alter the formal boundary between Block I and Block II, approve a polity biography, name a plague, or make one local story the cause of the Exodus. Late-Chain material ends in the Closing-Court crisis; warning, plague, breakout, Glass passage, and the Orphaned World remain Exodus material even when inherited practices cross the review boundary.

### Eight Provisional Anchor Polities

| Nursery polity | Closing pressure | Necessary human institution | Exploratory collapse outcome | Possible 1360 AR trace |
|---|---|---|---|---|
| **The Black Orchard Courts** | Ansharan Drinking Wars, purity claims, and household closures turn blood archives, hidden gardens, and service kin against one another without receiving a direct prophetic plague | orchard workers, blood-record clerks, household servants, burial keepers, healers, and defecting client families | courts consume rivals, seal themselves, flee bodily, or collapse into predation; some hidden households join Exodus columns, others remain, and some disappear inside the Drinking Wars | blighted blood-orchards, folded estate petitions, contradictory household laws, and survivor customs for testing an ancestral Patron claim |
| **The Cloudstep Principalities** | Qianglong caste recurrence, mixed-dynasty purges, and rival interpretations of the Four Horsemen force mountain princes and client estates to choose between court order and human release | terrace healers, seed keepers, midwives, pass wardens, stable workers, and foster households | some principalities open passes or defect, some purge mixed houses, and some retreat into recurrence vaults that fail or preserve only copied office-patterns | highland medicines and crops, conflicting caste inscriptions, sealed pass shrines, and lineages remembered as both traitors and liberators |
| **The Reed Crowns of Sarrakai** | Ishkatar's River-Leaves-the-Ledger crisis makes rotating crown, canal title, debt, and adopted kinship point toward different rulers | canal crews, water judges, matrilineal record keepers, boat households, and refuge wardens | crowns choose separately whether to open canals, defend royal debt, or fight for the Golden title; some cities flood, some dry, and some become human successor polities | incompatible flood books and canal charters preserving several simultaneous Pharaoh-Serpents |
| **The Ash Calendar Cities** | Xolotl's sacrificial calendar, Blood Sun economy, and rival city reckonings make missing days and refused offerings into imperial treason | calendar keepers, observatory workers, captive-exchange registrars, causeway crews, cooks, and funeral fraternities | cities desynchronize rites, revolt, burn archives, or enforce emergency calendars; some are destroyed, while others survive as short-lived human or mixed successor cities | ash-sealed observatories, rejected day names, causeway scars, and calendars that agree on an eclipse but not on who refused the offering |
| **The Moon Harbors of Caelene** | Nythalum retracts promised berths while the Uncopied Name exposes labor inventories, dead rulers, genuine Continuants, and Reconstructions | harbor labor councils, engineers, manifest keepers, rejected-passenger households, pilots, and salvage crews | ports revolt, seize unfinished craft, aid the terrestrial remnant, or attempt doomed physical launches; none develops an independent lunar programme | moonward wrecks, receiving frames, physical navigation tables, and berth treaties whose claimed debt survives |
| **The Glassward Crownlets** | the Prophet's gathering reaches a frontier of wells, slave depots, estate towns, and shifting client crowns as pursuit coalitions form behind it | oasis wardens, gate clerks, water carriers, refugee registrars, cart crews, and local burial fraternities | some gates and wells open, some rulers sell refugees, some households join the host, and some crownlets remain as precarious successor states | a lawful gate commission that operated once, rival liberation hymns, and fortifications bearing mutually hostile verdicts on the same ruler |
| **The Ossuary League of Urum** | plague burial, corpse exchange, ancestor custody, Titan fragments, and royal copy claims become impossible to adjudicate under imperial contracts | burial fraternities, seal-keepers, continuity auditors, physicians, and neutral hostage judges | league cities divide over whether to serve imperial claimants or protect the human dead; some close their gates while others preserve the distinctions needed after collapse | sealed keys and mortuary labels distinguishing a human grave, Giant body, Titan fragment, office relic, and empty royal Reconstruction |
| **The Seven Furnace Cities of Orqad** | every horizon demands weapons while a royal coalition attempts to assemble enough Titan fragments to monopolize the league | furnace guilds, fragment custodians, undertakers, hostage judges, and workers who know the separation constitution | the reconstitution crisis draws a bounded severance; individual cities seal, move, lose, or retain their fragment and therefore do not share one fate | weapons turning toward missing body pieces, sealed furnace constitutions, and one heatless city whose fragment may still live elsewhere |

These are approved **nursery anchors**, not promoted states. A developed episode must still choose one locality, social pressure, and carrier rather than treating an entire polity as one character.

### Five Provisional Ancient-Actor Slots

| Slot | Provisional nursery pressure | Required continuity ruling | Permitted future use and trace | Boundary |
|---|---|---|---|---|
| **Bound** | an Orqad fragment crisis, with any later Urum custody dispute treated as part of its afterlife | one `titan-continuity`, with every separated piece and seal treated as part of one still-accountable Titan-Firstborn | a sealed furnace, divided relic, prison-work, or modern reassembly threat | binding proves neither death, redemption, nor harmlessness |
| **Escaped** | a Black Orchard provincial court, Cloudstep cadet, or another lesser court carrying a refuge office into the Egg White | genuine bodily crossing under `giant-continuity`, kept distinct from office, residue, Mask, and later Patron claims | the actual escaped Giant, whose refuge, bargains, and later Patron-like dealings still have political consequences | the actor is not Anshara's final sovereign; escape makes no Giant a god, saint, human soul, or automatic Patron |
| **Counterfeit** | a court whose ruler is dead or interrupted but whose alleged escaped ancestor or Patron claimant remains politically useful | resolve the claimant as `constructed-pattern`, `infrastructure-office`, `residue`, `legend-persona`, `idol-pattern`, or crafted performance | a returning ancestor with correct secrets and a functioning anchor but failed continuity evidence | no copy becomes the original through acclaim, office, memory, patronage, or long performance |
| **Lunar** | Nythalum and the Moon Harbors | one genuine, uninterrupted `giant-continuity`, established from physical transfer evidence | a Moonbase Continuant whose choices, receiving frames, and ancient berth debts still matter | every route and survivor remains physically lunar, never a concealed Egg White ascent or post-interruption Reconstruction |
| **Repentant** | a Glassward, Reed-Crown, or Cloudstep Giant ruler defecting through a bounded release demand | one High or Dynastic Giant retains `giant-continuity`; repentance does not manufacture the Image or change creaturely kind | temporal mercy, opened gates, testimony, restitution attempts, and consequences that outlive the actor | one obedient act validates only that warning and never erases victims or the rest of the reign |

The five slots are approved planning interfaces, not approved actor biographies. Selecting an identity later must include a polity, carrier, continuity proof, historical consequence, and competing 1360 interpretation.

### Inherited-Practice Braid

The three principal companies are braided by **practices**, never by a secret organization, guaranteed bloodline, or continuous membership roll:

| Bed | Practices received or developed | What may pass forward | What must not be inferred |
|---|---|---|---|
| **The People Beneath the Bowl** | service-door routes, concealed household registers, hidden stores, false-death classifications, burial warnings, and refuge etiquette | anonymous techniques can leave Anshara through servants, defectors, stolen songs, and people who never knew the original refuge | the Bowl is not a universal resistance command and need not survive as one company |
| **The Company of the Broken Measure** | public measures, contradiction testing, sick-child movement, estate-to-estate conduct, and local invalidation of predatory office | ledger practice, accountable rationing, escort custom, and particular feeder roads can reach the Prophet's gathering | the company neither discovers one universal Giant weakness nor secretly organizes the Exodus |
| **The Last Cart** | rearward registers, burial lists, wheel-song, vulnerable-person duties, route correction, and public ration measures inside the Glass Ark | these practices become camp offices, departure customs, and later disputed memories among several peoples | similarity does not prove that Bowl or Measure members rode the Cart, founded every people, or transmitted one hidden revelation |

### Light Bridge Leads

These are trace-level leads, not additional signature companies, succession candidates, or additions to the developed-card count.
...
## Lane I-A — Human Heroes and Companies

### I-H01 — The People Beneath the Bowl

- **Era and scope:** deep Ansharan time through the Closing Courts; a changing refuge network inside one blood-ranked palace complex, not one immortal company or continental rebellion. Later keepers inherit practices without inheriting a complete membership roll.
- **Nursery provenance:** existing Garden bed; preserve its three firm roots unless deliberately revised.
- **Humble ground:** household slaves from unrelated peoples hide in service passages after a clerical error records them as dead.
- **Scriptural lens:** the hidden remnant; the vulnerable household protected under tyranny; deliverance beginning below official notice.
- **Comparative grammar:** outlaw refuge, household cunning, African and Indigenous emphasis on kin made through obligation rather than blood, and the company whose name is invented later.
- **World Egg torque:** Giant legal and magical systems cease recognizing the first fugitives because the estate's naming infrastructure has declared them dead. They gain no invisibility; doors, rations, burial mechanisms, and surveillance offices simply treat them as residue or property already written off. Later households deliberately reproduce only parts of that condition while adding hidden stores, burial warnings, service-route custody, and rules for receiving strangers.
- **Continuities:** living members are `human-soul`; the palace registry is `infrastructure-office`; later apparitions may be `residue`, not proof that the fugitives' souls remained.
- **Present-day traces:** children's games called *Beneath the Bowl*; an empty place at certain meals; service passages that open only when a person is formally recorded dead; hidden ration marks later mistaken for Exodus camp signs; several incompatible descendant claims.
- **Future hooks:** some keepers may reach a Glassward side-column, some may hold the refuge during the Drinking Wars, and some may be consumed by the closed court they tried to outlast. The empty seat may remember a traitor, rescuer, person sent toward the Prophet, or somebody who never returned.
- **Protected boundaries:** the network does not cause the general liberation, become the Prophet's hidden command, discover a master weakness of all Giants, explain every later escape road, or identify Anshara's final sovereign or capital. Similar practices in the Broken Measure and Last Cart beds do not prove continuous organization.
- **Source and sensitivity note:** use the grammar of hidden households and fugitive communities, not imagery from a single historical slave system. “African and Indigenous” is a research queue here, not a source citation; promotion requires a named community or text. Avoid making enslavement merely picturesque adventure furniture.
- **Review disposition:** `retain` — deepen as the long Ansharan communal-resistance ecology.
...
- **World Egg torque:** at a Qianglong naming station, officials compare the infant's measured body, inherited line-mark, first spoken name, and ledger entry. When all four agree, estate doors, ration stations, work rosters, and recurrence offices treat the child as belonging to the assigned caste. The midwives do not make children magically invisible: they introduce different controlled errors into different records, move the child before the cross-check, and keep witnesses who can later restore the true relation. During the Four Offices crisis, their separate household lists can be compared against estate ledgers; a person present in several independent households but only one recurrence roll is treated as living testimony rather than a returning caste office.
...
- **Continuities:** the ordinary dead human soul has routed; stolen remains carry `residue`; a reclamation intelligence is `constructed-pattern`; a true soul-cage is `bound-human-soul` and must be individually established; Giant or Titan remains require their own continuity proof.
- **Present-day traces:** an illegal ferryman's token found in pre-Exodus and early Exodus graves; funerary workers tapping a bier three times before transport; a chain of nameless burial caves whose routes do not form one geography; one Urum key-type copied far beyond the League.
- **Future hooks:** one stolen corpse may have contained a genuine bound soul; a respectable mortuary guild may descend from the betrayer who regularized the thefts; a later saint's body may have been misidentified through this network; one branch may remain behind to serve a human successor city rather than enter the Glass.
- **Protected boundaries:** burial protects dignity and route-integrity but does not save, damn, or command the human soul. Do not make every animated ancient corpse its original person.
- **Source and sensitivity note:** preserve the domestic and civic courage of burial piety. The broad African/European comparison is only a research queue; later expansion must name its sources. Avoid treating funerary practice from any living culture as occult technology.
- **Review disposition:** `retain` — deepen through Ishkatar, Urum, plague burial, and post-collapse continuity audit.

### I-H04 — The Company of the Broken Measure
...
## Lane I-B — Land, Folklore, and Local Encounter

### I-L01 — The Ford with Two Owners

- **Era and scope:** undated post-Fall, pre-Giant human memory; deliberately too small to fix Creation chronology.
- **Ordinary problem:** two households need to cross the same seasonal river while carrying seed, children, and the body of an elder.
- **Scriptural lens:** stewardship after lost ease; burial, hospitality, and neighbor-duty before kingship.
- **Comparative grammar:** wilderness as jurisdiction, river marriage and ownership tales from several European and African traditions, and Indigenous emphasis on place as relation rather than empty resource.
- **World Egg torque:** the ford lies where a river place-mind and an old road-law overlap. Each recognizes a different valid price. The households survive only after realizing that payment to one does not establish moral or metaphysical title over the other.
- **Continuities:** humans are `human-soul`; the river and road manifestations are `court-law`; later carved speakers may be `residue` or constructed shrine mechanisms.
- **Present-day traces:** paired offering stones on some fords, one wet and one dry; the legal maxim *a crossing has more than one owner*; funeral parties crossing in two stages.
- **Future hooks:** later kings use the maxim to justify layered sovereignty, and rival courts claim one household founded their law.
- **Protected boundaries:** do not declare this the first Menhir bargain, the origin of all river ritual, or an eyewitness account of Eden. Menhir acceptance is consequence, not moral approval.
- **Source and sensitivity note:** the African and Indigenous comparison is a research queue rather than approved attribution. Before expansion, select and cite a named tradition; borrow relational place-law, not a living water spirit, ceremony, or sacred geography.
- **Review disposition:** `defer` — preserve as a protected pre-Giant etiological bed until named-source and prehistory review.

### I-L02 — The Hunter Who Knew He Was a Guest

- **Era and scope:** Age of Chains; a southern forest-cycle that later Folk traditions may inherit without proving direct ethnic continuity.
- **Ordinary problem:** a condemned hunter flees a Giant census into woodland his pursuers call unclaimed.
- **Scriptural lens:** the fugitive spared outside imperial jurisdiction; humility before a law one does not own; wisdom exceeding force.
- **Comparative grammar:** Yoruba hunter diplomacy, Isles fairy guest-right, Slavic and Baltic forest law, and the warning-ignoring culture hero corrected by kin.
- **World Egg torque:** each path, grove, bridge, and hunting ground belongs to a different `court-law` continuity. The hunter earns bounded guest-right through funeral attendance, truthful naming, food refusal, and settlement of a dispute. Giant soldiers are not attacked as monsters; they are successively held for trespasses their own categories cannot perceive.
- **Continuities:** hunter and pursuers are `human-soul` or nonhuman Giant according to participant; local powers are `court-law`; a speaking animal may be `biological-pattern` under court alteration, not automatically a god.
- **Present-day traces:** seven contradictory forest courtesies, hunting lodges with a guest chair facing outward, and a road on which armed officials still remove insignia.
- **Future hooks:** one court may still recognize the hunter's descendants as guests; the final obligation may remain unpaid; an ancient Giant prisoner may still be held because its case was never heard; the refuge may later shelter an independent survivor community that never joined the principal host.
- **Protected boundaries:** the forest is a polity, not an exotic monster biome. The hunter does not conquer or convert it, and its laws are not divine justice.
- **Source and sensitivity note:** do not label the grammar generically African. If later developed, research a particular hunter tradition while rebuilding every sacred name and ritual surface.
- **Review disposition:** `defer` — reserve for the outside-host survival ecology after regional, polity, and named-source placement.
...
- **Continuities:** child and sibling are `human-soul`; Father Sun remains continuous Menhir `yolk-sovereignty`; the captured light-bird is `constructed-pattern` attached to an `infrastructure-office`; imitative daylight creatures may be `biological-pattern` or court-altered fauna only where separately established.
- **Present-day traces:** eclipse games in which the youngest child cuts a cord; noon bells omitted on one summer day; several Giant observatories containing a one-child gap in their reliefs.
- **Future hooks:** the sibling may be the real engineer, the dismantled light-bird may retain one obsolete command, and some traditions claim the child later became the Sleeper. Xolotlan liberation accounts may treat the incident as proof that the artificial noon could fail long before the Blood Sun crisis.
- **Protected boundaries:** do not injure or bind Father Sun in sovereign fullness, make the child cause the later plague or Blood Sun explosion, explain an astronomical Weird, or copy an Innu sacred figure's name, family, sequence, or cosmological role.
- **Source and sensitivity note:** retain only the grammar of reckless small hero, sibling warning, capture, and world consequence. Nation-specific Innu material requires community-led sourcing before expansion.
- **Review disposition:** `revise` — retain the child-and-sibling cycle after replacing captured Menhir manifestation with a bounded blood-calendar construct.
...
- **Continuities:** thief is `human-soul`; calendar is `infrastructure-office`; any later laughing persona is `residue` or a culturally grown Mask, not automatically the thief.
- **Present-day traces:** leap-feasts, unscheduled market days, a children's custom of stealing breakfast, the legal phrase *work done on no king's day belongs to the hands that did it*, and physical harbor manifests with one shift no royal column can claim.
- **Future hooks:** rulers repeatedly try to annex the interval; labor councils may use its memory during berth revocation; a Reconstruction or Patron claimant may offer to restore the old schedule in exchange for renewed title.
- **Protected boundaries:** this is not the origin of cosmic time, sabbath itself, universal freedom, Nythalum's launch failure, or an Egg White current. It cannot rewrite judgement or make belief sovereign over chronology.
- **Source and sensitivity note:** Ananse is specifically Akan; “Raven” spans distinct Indigenous nations and remains a research queue until one community and collection are named. Do not use them as generic interchangeable tricksters. The only transferred element here is impossible procurement producing unintended public benefit.
- **Review disposition:** `revise` — retain as a labor-calendar and berth-office cycle within Nythalum's exclusively physical Moon programme.
...
- **World Egg torque:** one river freezes upstream and downstream in different seasons, one forest carries two incompatible guest-laws on alternating nights, and one mountain casts its legal shadow toward rather than away from the sun. These are damaged overlaps among created natural laws, not new cosmological principles.
- **Continuities:** Menhir remain enduring `yolk-sovereignty`; local manifestations and place-minds are `court-law`; the scars are persistent law-damage rather than persons, souls, or new Menhir.
- **Present-day traces:** three surveyed anomalies, nine contradictory victory tales, and a Faewild road that treats each account as a valid password but none as moral truth.
- **Future hooks:** which Menhir fought at each border, whether all three scars belong to one campaign, and why Giant maps omit the same interval remain open.
- **Protected boundaries:** add no mortal company; do not explain the Glass, bird-marked sites, or origin of natural law; do not make Menhir hospitality divine approval; do not settle the Schism's full sequence through these scars.
- **Source and sensitivity note:** divine-war landscapes from living Hindu tradition require named-text research before expansion. No Greek, Norse, or Hindu deity, sacred place, kinship, or signature battle is transplanted into authorial cosmology.
- **Review disposition:** `defer` — preserve the Faewild scar cluster outside the Unbinding until its separate source and Schism review.

## Lane I-C — Institution, Memory, and Relic

### I-M01 — The Names Beneath the Numbers

- **Era and scope:** entire Age of Chains into the gathering years; many independent practices later mistaken for one resistance archive or one Prophet-directed census.
- **Ordinary origin:** enslaved clerks record private names beneath removable wax, inside loom weights, and in the blank spaces of Giant tax numerals.
- **Scriptural lens:** genealogical memory under exile; the Lord's knowledge of the erased person; scribes preserving identity against imperial deletion.
- **Comparative grammar:** performed and custodial history, hidden-name folklore, and TES-derived separation of person from public definition.
- **World Egg torque:** estate transfers normally use a work number as if it were the person: the number moves rations, sleeping space, punishment liability, and access permissions even when the human bearer has died or escaped. A hidden personal name does not stop a guard or create a soul. It gives later witnesses something independent to compare with household, burial, and work records. When those records disagree, transfer offices can no longer treat one number, recurrence performance, missing captive, and royal copy-claim as the same person without exposing the contradiction. During the Unbinding, human registrars use those contradictions to restore particular people to the gathering rolls and to reject several estate claims.
- **Continuities:** named persons are `human-soul`; tablets and offices are `infrastructure-office`; later speaking archives are `residue` or `constructed-pattern`; a true bound person requires separate proof.
- **Present-day traces:** numerals written with a tiny downward stroke, hidden name-lists in foundations, disputed gathering-roll fragments, and arguments over whether a dynasty's oldest ancestor was a person or work category.
- **Future hooks:** a newly found list could restore an inheritance, expose a saint's suppressed ancestry, reveal that two famous heroes shared one estate-number, or prove that a supposed Nythalum ancestor appears only as a post-interruption Reconstruction.
- **Protected boundaries:** names influence access and memory but do not create the Image, guarantee salvation, or prove that every person in a list survived.
- **Source and sensitivity note:** avoid romanticizing archival survival as a substitute for the violence of bondage. Do not copy a specific historical slave-name practice without contextual research.
- **Review disposition:** `retain` — deepen as a plural preservation practice feeding, but never secretly controlling, the remnant registers.

### I-M02 — The Song of the Nine Service Doors

- **Era and scope:** deep Giant time through the gathering years; a family of work songs that may combine several palaces and centuries rather than one unaltered composition.
- **Ordinary origin:** kitchen, laundry, furnace, nursery, stable, corpse quay, garden, archive, and drain workers each contribute one verse encoding a safe passage.
- **Scriptural lens:** communal lament becoming deliverance-memory; many members with different gifts forming one living body; song preserving truth when monuments lie.
- **Comparative grammar:** Mandé and Nyanga performed history, Baltic song-memory, labor-song route keeping, and company epics controlled by multiple custodians.
- **World Egg torque:** the song does not cast a spell by itself. Correct performance aligns remembered directions with surviving service architecture, giving a particular route enough metaphysical mass to appear through later renovations and partial ruins. Refugees and gathering crews can carry route-keeping technique beyond the palace while changing words, duties, and cadence for new roads.
- **Continuities:** singers are `human-soul`; route is `infrastructure-office`; its accumulated echo is `residue`; any autonomous guide must be separately classified.
- **Present-day traces:** nine occupational guilds each preserve a verse they claim is metaphorical; a ruin opens only when verses are sung in a disputed order; children's versions omit the corpse-quay stanza; some Glass-road hymns preserve its call-and-answer duties without its melody.
- **Future hooks:** one lineage was erased and its missing verse may correspond to a geography that no longer appears; rival cities claim ownership of the final cadence; whether the Road Sung Across Glass learned from one branch or independently solved the same human problem remains open.
- **Protected boundaries:** performance sustains access, not souls; no singer lineage owns divine revelation; the song cannot become a master map of all Giant sites or a competing revelation of the Glass road. Resemblance does not prove one continuous company.
- **Source and sensitivity note:** the named research leads are Mandé jeli traditions and the Nyanga Mwindo epic, each requiring its own dossier. Performed history is intellectual and legal custody, not primitive superstition. Do not lift a living epic's formulae, performer titles, sacred names, or custodial claims.
- **Review disposition:** `retain` — deepen as inherited route practice bridging palace refuge and later camp technique.
...
### I-I01 — The Seven First Faces

- **Era and scope:** immediately after Yolk fusion; only later apocryphal traditions attempt to narrate it.
- **Seed function:** establish that an Idol's original office-holder did not emerge from fusion as a simple enlarged angel. Cohort, betrayed commission, Shadow ecology, and sovereign personality reorganized into the first Regnant expression.
- **Scriptural lens:** created spirits possessing offices as property; rebellion preserving culpability; corruption requiring a real good to betray.
- **Comparative grammar:** succession myths and wars of divine generations used only as an in-world interpretive form; TES-style distinction between enduring role and organizing personality.
- **World Egg torque:** each Idol's first face is an executive crystallization inside one continuous guilty `yolk-sovereignty`. It is **formation, not succession**, and gives no mortal a reusable route to apotheosis.
- **Continuities:** Idol is `yolk-sovereignty`; projected first images are `idol-pattern`; original constituent spirits are not detachable `created-spirit` persons after completed fusion.
- **Present-day traces:** incompatible creation hymns in which each Idol appears first as an angel, throne, wound, city, animal, or plurality; none is a direct witness.
- **Future hooks:** an Archmask may claim to remember a moment before the first face stabilized; that claim can be sincere, manipulative, or inherited from cohort sediment.
- **Protected boundaries:** do not make the Throne neutral, erase original guilt, create peer gods, or authenticate the Atherian claim that mortal dreams created the seven.
- **Source and sensitivity note:** divine-generation cycles from Greek, Norse, and Hindu material may shape mortal misunderstandings but should not be reproduced as authorial cosmology.
- **Review disposition:** `defer` — preserve as primordial Idol-formation control, outside the first Unbinding history pass.
...
- **Conflict grammar:** Wrath. The challenger inherits every grievance used to justify its war even though it fails to become Regnant.
- **World Egg torque:** Giant institutions teach Wrath a new cultural face able to present vengeance as protection of the enslaved as well as domination by the master. The Archmask exploits the civil war and failed sacrifice architecture but does not cause the plagues, implant hatred, or control every combatant; a true succession is unnecessary.
- **Continuities:** Wrath remains `yolk-sovereignty`; contender must be an autonomous Archmask rather than a dependent `idol-pattern`; Giant office is `infrastructure-office`; no human soul becomes the Red King.
- **Present-day traces:** two incompatible sequences of Xolotl war masks; Grobi traditions naming both the Fury and the King-Who-Lost-His-Face; volcanic temples rebuilt around an older sealed dais; court records that disagree whether the Red King defended captives, requisitioned them, or did both in succession.
- **Future hooks:** the defeated claimant may survive as a regional Archmask; restoration cults may believe the old Wrath was merciful because it hated different enemies; one human war-company may have accepted its protection and later concealed the price.
- **Protected boundaries:** do not make this the cause of all Giant war, the Blood Sun failure, the Great Prophet's victory, or the origin of Wrath. It remains an Archmask revolt unless the Block-I candidate slot is explicitly reopened.
- **Source and sensitivity note:** do not equate Mesoamerican-coded blood imagery with inherent savagery or use a living Indigenous religion as demonic shorthand. World-native Xolotl material must bear its own setting history.
- **Review disposition:** `revise` — retain as a bounded Xolotl Closing-Court Archmask revolt after its causal limits are fixed.
...
- **Present-day traces:** Giant banquet halls with two superimposed central mouths; ghoul liturgies that begin with an apparently nonsensical coronation-by-eating; a suppressed Funeral Thieves verse naming *the mouth before the Mouth*.
- **Future hooks:** the consumed former persona may still speak from within the Long Table; a cult may seek to make it vomit up an older, supposedly purer ruler.
- **Protected boundaries:** no Regnant can redeem the Idol; appetite and feasting remain created goods until deliberately distorted; do not make famine victims morally responsible for Gluttony's strength.
- **Source and sensitivity note:** avoid body-size stigma, eating-disorder caricature, or treating hunger itself as vice. The feeding architecture is deliberate denial and status consumption.
- **Review disposition:** `defer` — Giant continuity is now settled, but the sole Block-I succession candidate remains dormant until the separate Idol review and whole-history succession cap are applied.

### I-I04 — The First Shared Loaf

- **Era and scope:** late Chains into the first food and granary failures; a Counter-Countenance trace preserved in mutually hostile slave, Giant, and later camp accounts.
- **Ordinary event:** during a siege-famine, captive cooks distribute one ration by publicly witnessed need rather than reserve it for the strongest escapees or the estate offices that claim priority.
- **Scriptural lens:** manna and provision without hoarding; hospitality; the widow's small offering; faithful action that does not guarantee immediate rescue.
- **Comparative grammar:** communal food as covenant, trickster feast reversal, and the quiet person whose act matters more than the warrior's.
- **World Egg torque:** the cooks place the complete ration and the complete household list before witnesses, then cut portions by declared need. Within the Gluttony overlay, an ash-dark place appears at the table for every person deliberately omitted from the estate roll, and locked reserve bins marked for those missing names become visible as part of the same ration system. The Bread-Bearer does not create extra food: the physical loaf still yields only its actual weight. The cooks use the exposed omissions to open one concealed store, correct one ration board, and break the banquet's rule that only ranked mouths count as present. Later camps can copy the public weighing and complete-name procedure, but they cannot reproduce the apparition on demand.
- **Continuities:** cooks are `human-soul`; Counter-Countenance belongs to Gluttony's `yolk-sovereignty` as suppressed witness; any loaf apparition is temporary `idol-pattern` under that exceptional condition, not a saint or pact source.
- **Present-day traces:** famine tables leave one loaf uncut until every person is named; a Giant kitchen floor bears handprints around a deliberately empty central plate; some Exodus ration boards preserve an empty line that later traditions attribute to this meal without proving transmission.
- **Future hooks:** later cults may falsely worship the Bread-Bearer; the surrounding Gluttony network may capture those rites; a saint's life may absorb the act and erase the cooks; one household that obeyed the sharing rule may survive a bounded food sign while still answering for other crimes.
- **Protected boundaries:** the Counter-Countenance grants no repeatable spell, cult, absolution, immunity, endless multiplication, or method for converting Gluttony into a benevolent power. It does not replace the Prophet's provision or import Eucharistic clarity into the pre-Redeemer age.
- **Source and sensitivity note:** retain the moral and social meaning of shared nourishment without copying Eucharistic language into the pre-Redeemer age.
- **Review disposition:** `retain` — deepen as one bounded famine, public-measure, and Counter-Countenance trace.

---

# Block II — Exodus through the Age of Stars

## Block-II Density and Protected Boundaries

| Era | Human/company | Land/folklore | Institution/memory | Metaphysical pressure |
|---|---:|---:|---:|---:|
| Exodus | 1 retained Garden company | 1 route seed | 1 trace cluster | Idol pressure only; no succession required |
| Scattering | 2 | 1 | 1 | Mask-turnover probes; no authorial succession candidate |
| Hearths and First Kings | 2 | 2 | 1 | 2 |
| City-States | 1 | 1 | 1 | 2 |
| Stars | 1 | 1 | 1 | 1 continuation of earlier pressure |

Protected boundaries:

- The Great Prophet remains the central liberator. Supporting companies reveal how mass deliverance survives; they do not replace his commission or miracles.
- The Prophet's failure, death, sanctuary refusal, and disputed responsibility remain contested as established. No seed resolves them.
- The Year of Standing Stars remains unresolved. Seeds may leave records damaged by it but cannot explain or exploit its cause.
- The Scattering produces distinct peoples through long migration, settlement, intermarriage, choice, and loss. No seven-person company literally fathers every nation.
- Menhir bargains remain exact amoral law, not alternate revelation.
- Idol pressure exploits human choices and institutions but does not cause every war, kingdom, debt system, or religious drift.
- Atherian Star powers are real Menhir encounters filtered through civic theology. Their reality does not prove Atherian genealogies or moral claims.

### Exodus Peripheral Cast — Trace-Level Leads

These unreviewed leads deepen the people around the Great Prophet without creating a second Prophet or another signature company. They are not counted among the developed cards until expanded through [[Historical Seed Card Template|a full seed card]].

- **The mismatched sanctuary artisans:** dyers, joiners, smiths, carriers, and displaced temple laborers make portable fittings from Giant salvage; in 1360 AR, three churches preserve pegs that fit one another but disagree about the lost structure.
- **The rearward adviser:** an elderly provisioner repeatedly urges the main host to slow for the vulnerable rear and is remembered as coward, prophet, quartermaster, or Last-Cart partisan; no miracle or universal office is implied.
- **The hired curse-seer:** a Giant client purchases a curse against the refugees, but the seer's pack animal recognizes the dangerous messenger first and the commissioned words emerge as warning or blessing; the seer remains morally compromised and never becomes a rival revealer.
- **The internal returner:** one camp leader sincerely prefers measured bondage to hunger and uncertainty, steals rations to force a return, later repents, and leaves a confession whose use in collective blame is forbidden.
- **The failed scout:** the Praise roll remembers a guide who admits the wrong road before status kills the rearward host; this may later join the Seven or remain an unrelated ordinary correction.
...
- **Era and scope:** Exodus; the vulnerable rear of the federated Glass Ark during convergence, pursuit, and the desert stages.
- **Nursery provenance:** existing Garden bed.
- **Humble ground:** refugees assigned to a broken provision cart fall behind and must guide elderly, injured, pregnant, unwanted, quarantined, and slow-moving people whose access to the Prophet's most visible signs comes through camp offices rather than proximity to him.
- **Scriptural lens:** the mixed multitude, care for the vulnerable, provision in wilderness, and holiness expressed through carrying rather than acclaim.
- **Comparative grammar:** humble travelling company, dangerous road, performed route-memory, and the member who simply goes home.
- **World Egg torque:** the rear crew uses a call-and-response wheel-song as a moving roll call. The front caller names the next person, duty, ration point, burial place, or route correction; the rear caller confirms the last one actually passed. In stretches where Glass reflections show duplicate carts or false tracks, a reflection can repeat a melody but cannot supply the current names, injuries, deaths, and work changes known at both ends of the real column. A failed response tells the crew exactly which interval to search. This keeps a bounded rear section socially connected; it does not reveal a supernatural compass or cross the desert by itself. High signs answer only impossible bottlenecks, while people still provision, repair, treat, bury, defend, and update the song.
- **Continuities:** company are `human-soul`; cart and route may become `infrastructure-office`; later wheel apparitions are `residue` unless a true saintly commission is established.
- **Present-day traces:** damaged wheel or wheel-loaf at funerals and departures; several peoples claiming different passengers; paired rear-and-front names in road rites; cart tracks appearing in art before wheeled transport is otherwise attested locally.
- **Future hooks:** one passenger may found an institution, another may be omitted as a burden, one crew member may have served an earlier feeder road, and one version says the Last Cart reached the sanctuary threshold first and waited. Whether the “cart” was one vehicle, a rotating rear office, or several carts compressed into one company remains open.
- **Protected boundaries:** the company does not independently cross the whole Glass, command the hidden Chain-era networks, contradict or replace the Prophet, monopolize the provision signs, or explain the Glass Weird. Shared practices do not establish shared membership.
- **Source and sensitivity note:** retain the scriptural concern for the vulnerable without reproducing Israelite tribal identities one-to-one.
- **Review disposition:** `retain` — deepen as the human rear office of the Glass Ark while keeping membership and final outcomes provisional.
...
- **World Egg torque:** each locality grants guest-right to particular named people after a particular act—repairing a ford, burying a local dead person, keeping a night watch, or accepting a household obligation. Its cairn, gate, ferry, or path then admits those named guests under that condition. Later songs compress spouses, guides, hosts, dead companions, and several parties into “the Seven,” but the old routes still answer only to their original local lists. A traveler invoking the shared company-name may therefore reach a well while the companion beside them is returned to the previous cairn. The farther the composite story spreads, the less any one historical route can carry everybody it now remembers.
...
- **Era and scope:** Scattering; one migrant household later claimed by several river and oasis communities.
...
### II-L01 — The Road Sung Across Glass

- **Era and scope:** Exodus; one rearward route tradition, not the canonical explanation of the crossing.
...
- **World Egg torque:** through one dry season, migrants and established households clear silt, repair the lining, ration draws, carry water to people unable to reach the well, and leave one lower basin untouched for recovery. After that repeated care, four actions produce consistent local results: a first freely given cup keeps the public spout clear; the agreed labor or goods keep the draw channel open; crossing the protected basin clouds the water; and failure to repair closes the offending household's side channel until witnessed restitution. These effects grant no moral verdict or eternal title. They describe how this one well responds to its compact.
...
- **World Egg torque:** the boundary follows repeated seasonal use rather than a surveyed line. Each time surveyors set a permanent marker, thawing ground, roots, or floodwater returns it to the edge actually used that season; replacing it with a heavier stone damages pasture and blocks the old watercourse. The effect stops at this locality and does not select a morally rightful owner. The imperial road remains stable only after its charter leaves one stated interval without permanent milestones and the affected households negotiate dates for grazing, cutting, passage, and repair.
...
### II-M01 — The Wheel Behind the Prophet

- **Era and scope:** Exodus memory complex growing from the Last Cart rather than a separate company.
...
- **World Egg torque:** the ring is carried through the winter kitchen before the crown. During the rite, named storehouses must be opened, a public ration served, and declared refuge places made accessible. If officials recite the obligations without actually releasing the food or opening shelter, the suspended ring twists toward the closed store or doorway and cannot be carried level into the coronation hall. Copies can acquire the same bounded response where generations repeat the complete public procedure. The ring tests provisioning and shelter in that rite, not royal blood, private virtue, or conduct elsewhere; a morally corrupt ruler can still satisfy it.
...
- **World Egg torque:** the synthesizers sometimes identify records of the same enduring Menhir under different civic names. They then translate seasonal opposition, shared symbols, treaty alliance, and recurring conjunction into literal marriages and parentage. The new genealogy gives cities a common calendar, equivalent priestly titles, recognized festival embassies, standard marriage formulas, and a shared order for invoking bounded civic offices. Those practical agreements let institutions work across borders and give the genealogy political force. They do not make the claimed births or marriages true of the Menhir themselves.
...
- **Era and scope:** Hearths and First Kings; sole Block-II Pride succession candidate in this pass.
...
- **World Egg torque:** the relevant courts leave one witness position legally empty so a dynasty, ancestor office, or ruler can assent on behalf of a spouse or hostage. Repeated ceremonies let dependent Lust Masks use that unassigned position: the faceless Guest speaks through the proxy, promises fertility, beauty, legitimacy, or diplomatic access, and treats the represented person's later refusal as breach of property. Courts with different portraits can produce related Masks because they repeat the same coercive legal act under Lust's Throne; the images need not be one traveler. Naming each person separately, recording revocable consent, and denying the proxy power to sign closes this local access without abolishing marriage or desire.
...
## Praise of the Remembered — Provisional Roll

This Sirach-style roll plants names and sayings more lightly than a full card. It is not counted among the thirty-eight developed seeds, creates no new signature company, and fixes no biography. Each line remains unreviewed; if promoted beyond a saying or trace, it requires a [[Historical Seed Card Template|full seed card]].

| Era | Provisional remembrance | Light carrier or future hook |
|---|---|---|
| Exodus | **Remember the woman who kept the coals dry when every honored brazier failed.** | Ash pouches in Last-Cart funerals; perhaps a portable-sanctuary dyer or cook. |
| Exodus | **Remember the failed scout who confessed the wrong road before pride killed the rearward host.** | A route-keeper's rule that a guide may reverse judgement without losing office. |
| Scattering | **Remember the child of the eighth well, who gave the stranger a ration already too small.** | A cup no genealogy can lawfully inherit; the child and stranger's later fates are lost. |
...
| I-L04 — Thief of the Unowned Day | `revise` | Nythalum labor-calendar and berth-office cycle | harbor locality and physical launch chronology |
...
| I-I04 — First Shared Loaf | `retain` | bounded famine, public measure, and Counter-Countenance trace | one locality and Gluttony-network classification |
| II-H01 — Last Cart | `retain` | rearward human office inside the Glass Ark | membership, camp stage, provision encounters, and ultimate outcomes |
| BX-01 — Petition of the Closing Court | `retain` | trace-level Giant court archive lead | anchor polity and addressee interpretations |
| BX-02 — House That Opened One Gate | `retain` | trace-level bounded-obedience lead | warning, polity, and temporal consequence |
| BX-03 — City That Stayed Behind | `retain` | trace-level outside-host successor lead | geography, infrastructure, and later-contact decision |
| BX-04 — Severed Pursuit | `retain` | trace-level Commission-of-Severance lead | local pursuit-work, witness carrier, and target mechanism |

### Bridge Review Gates

1. **Continuity gate:** retain `human-soul`, `titan-continuity`, `giant-continuity`, `constructed-pattern`, office, residue, legend-persona, Mask, and copy-claim distinctions in every developed episode. A Giant Reconstruction is a use of `constructed-pattern`, not a separate continuity.
2. **Polity gate:** choose one locality inside one of the eight nursery anchors before fixing a participant list or collapse sequence.
3. **Plague gate:** approve only broad imperial claims, warnings, protected acts, and collapse functions before giving any recension a numbered or canonical plague list.
...
1. The People Beneath the Bowl.
2. The Company of the Broken Measure.
3. The Last Cart.
4. The Seven Who Took the Wrong Road.
5. The Crownless Company.
6. The Wolf and the Lantern.
7. The Bent-Key Fellowship.
8. The Road without Milestones.

Their presence here does not alter the Garden's firm roots or promote ultimate fates. The matrix adds and reviews exploratory growth around those roots for later canon selection.

## Succession Candidate and Fallback Register

| Seed | Idol | First-pass status | Review treatment |
|---|---|---|---|
...
## Audit Questions for Every Retained Seed

1. Is the ordinary human problem still legible without the later grand fate?
2. Does the seed leave at least one material, institutional, narrative, and hostile trace without requiring all four?
3. Does its scriptural lens govern moral orientation without reproducing named persons and signature sequences?
4. Does its comparative grammar remain specifically sourced at expansion time?
5. Does the World Egg torque arise from established continuity, route, bargain, Mask, office, or metaphysical mass rather than a new one-off rule?
6. Have human choice and material history remained primary instead of an Idol causing everything?
7. Does the seed preserve the Lord's unique uncreated divinity, the Redeemer's unique event, and the human soul-route?
8. Does it avoid explaining any registered Weird?
9. If the supernatural interpretation is false, does a usable human or institutional story remain?
10. If it is true, can authorial notes distinguish `human-soul`, residue, office, Mask, Archmask, court-law, and sovereign continuity cleanly?
