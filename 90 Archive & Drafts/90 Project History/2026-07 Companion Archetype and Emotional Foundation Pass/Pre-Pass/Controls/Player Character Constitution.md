---
type: mechanic
status: archived
canon_level: obsolete
visibility: author-only
era: ["Present Day"]
region: ["Grenzburg"]
culture: []
related_factions: ["Blackjack Corps", "Medori Bank", "Duchy of Grenzburg"]
appears_in: ["Grenzburg"]
needs: []
aliases: ["Player Mandate and Dual Chain of Command"]
system_family: player
player_system: constitution
tags: [game-ready]
archive_reason: "Pre-archetype companion foundation snapshot retained for provenance."
archived_from: "02 Playable Settings/Grenzburg/10 Production Bible/Player Character Constitution.md"
archived_on: 2026-07-26
superseded_by: "[[Player Character Constitution]]"
---
# Player Character Constitution

This note is the controlling authority for Grenzburg's protagonist, character creation, social expression, moral tendencies, field capabilities, and vocation architecture. [[Player Systems MOC]] supplies the subordinate system references. [[Player Identity Reactivity Matrix]] controls how often and how materially the world recognizes those choices. Exact damage, timing, probability, and final balance remain provisional.

## Fixed Identity

The protagonist is a silent, customizable human outsider whose debt has been converted into service in the [[Blackjack Corps]]. They enter the region in a Medori debtor convoy on the northern road and are seconded to Grenzburg as a field specialist during the emergency.

The player is not:

- native to Grenzburg;
- a secret heir, foretold sovereign, or chosen savior;
- commander of the Blackjacks, ducal army, militia, or companions;
- automatically loyal to the Bank or Duke;
- free of debt at the beginning.

The Duke and his officers drive the public war effort. The Bank controls the player's service account, equipment access, and threat of collection. Bank pressure appears through specific assignments, faction work, consequences, and personal leverage rather than a private rider on every main quest.

## Silent-Protagonist Rule

The player never receives a fixed spoken voice or authored personality. Dialogue choices communicate intent, knowledge, temperament, and moral position without reducing the protagonist to slogans.

Important responses are concise, legible before commitment, and allowed to expose follow-up questions. Silence, refusal, deception, mercy, threat, cultural recognition, practical knowledge, confession, and principled disagreement are all valid forms of expression.

## Character Creation Contract

Character creation grants exactly these identity and play decisions:

1. appearance, body, voice absence, and sex;
2. one origin culture and its knowledge tag;
3. one additional cultural familiarity;
4. one background knowledge;
5. a primary and secondary social-stat focus, with the same choice twice creating a specialist;
6. one initial virtue or sin leaning, or complete neutrality;
7. one of five starting vocations.

The nine culture tags are Eclessian, Thalmyric, Sestaran, Jirahar, Ahl-e-Ahd, Folk, Ättar, Tarkhan, and Nsizwa. They represent learned familiarity, not racial instinct or guaranteed agreement with another person.

The nine background fields are Wilderness, Underworld, Military, Nobility, Scholarly, Merchant/Craft, Peasant/Labour, Mystic/Religious, and Drifter/Vagabond.

[[Character Creation and Identity]] controls the full sequence and presentation.

Every origin culture receives approximately ten meaningful campaign uses, every background eight, and every starting vocation eight. A meaningful use changes access, risk, price, trust, information, traversal, encounter behavior, or quest method. Recognition includes both opportunity and friction while preserving a critical-path workaround.

## Statistics and Social Resolution

Combat uses:

- Health and Stamina;
- Strength, Magick, and Stagger Power;
- Physical Defence, Magick Defence, and Staying Power.

Social play uses Charisma, Wisdom, Intelligence, and Luck. The four active social skills are Coercion, Diplomacy, Deception, and Insight.

Before a checked response is selected, the interface displays the governing skill or knowledge and a qualitative risk band: **Favored**, **Even**, **Risky**, or **Dire**. The primary roll, exact target, and Luck adjustment remain hidden. Failure normally changes price, trust, evidence, timing, hostility, danger, or compromise rather than terminating a quest.

[[Player Statistics and Resolution]] and [[Social Skills and Knowledge]] control these systems.

## Virtue and Sin Tendencies

All seven virtue and sin pairs remain visible in the character record. Sustained conduct moves a tendency through clear thresholds. One isolated choice does not redefine the protagonist.

Only major vows, defining commitments, or a deeply established incompatible character may close a choice. Ordinary dialogue is never randomly removed. The system describes the player's repeated conduct; it does not make every moral claim equally true.

[[Virtue and Sin Tendencies]] controls thresholds, feedback, incompatibility, and restoration.

## Shared Field Capability

Every vocation can sprint, jump, vault, grab ledges, perform ordinary climbing, swim, carry, throw, slide, make a stealth knockout, and climb large monsters. Every critical route remains completable with every vocation.

Vocation movement creates speed, safety, shortcuts, combat opportunities, and discoveries. It never becomes a mandatory key for the critical path.

The five shared trainable proficiencies are Locksmithing, Pickpocketing, Sneaking, Free Running, and Tracking.

[[Shared Movement and Physical Interaction]] and [[Proficiencies and Field Skills]] control field play.

## Vocation Constitution

Grenzburg uses third-person action combat. The player equips one authored or custom vocation at a time.

- Authored vocations are field-locked to their native weapons, armour, active skills, and signature mechanics.
- Character level and vocation mastery advance separately.
- The vocation active when a character level is gained supplies that level's combat-stat growth.
- Ordinary skills advance through mastery; trainers open disciplines, rank gates, and signature techniques.
- Vocation changes occur only at supported safe points outside immediate danger.
- Mastered account unlocks remain available to the current campaign and future new games where their original gate has been met once.

The five starting vocations are:

| Vocation | Native kit | Primary promise |
|---|---|---|
| [[Fighter]] | one-handed sword, axe, or mace; shield; heavy armour | guard, counter, stagger, and hold a line |
| [[Slayer]] | paired daggers or short swords; grappling knives; light armour | infiltrate, climb, reposition, and exploit weak points |
| [[Hunter]] | longbow; light field armour | track, prepare arrows, read monsters, and control range |
| [[Warlock]] | paired war-wands; robes or light armour | bind elemental Shell scripts while managing Arcane Burn |
| [[Medium]] | medium's rod; shade roster; robes or light armour | direct non-conscious combat imprints and shadow fields |

[[Vocation Progression and Mastery]] controls growth, switching, and account access. [[Equipment and Skill Loadouts]] controls authored equipment boundaries.

## Advanced and Custom Vocations

- [[Paladin]] unlocks during Bernard's Act II finale and receives its capstone during his Act III finale. Learned great-weapon handling remains available, but White-Flame techniques require moral coherence and genuine restitution after grave violation.
- [[Binder]] unlocks during Zafir's Act II finale and receives its capstone during his Act III finale. It may direct non-conscious bound forms and freely covenanted beings, never human souls or coerced persons.
- [[Bard]] unlocks during Ilyana's Act II finale and receives its capstone during her Act III finale. It sustains harmonic Shell refrains, recoverable-health support, elemental cadences, and bounded battle compulsion that never creates consent.
- [[Lancewright]] unlocks during Tavio's Act II finale and receives its capstone during his Act III finale. It uses a one-handed war spear and proof gauntlet to place, load, release, and safely Yield bounded elemental marks.
- [[Custom Vocation Workshop]] opens after two authored vocations are fully mastered. It is a metagame builder, not an in-world school. A saved custom build is itself one field-locked vocation.

Druid, Witch, Monk, and further advanced vocations are deferred. Their absence from Grenzburg's launch framework does not rule on their existence elsewhere.

## Metaphysical Safeguards

- Warlock and Lancewright obey [[Warlock Rune Magic]] and draw bounded elemental relations through the Shell.
- Medium shades are non-conscious combat imprints, never captured people or souls.
- Paladin sacred techniques obey [[White Flame Doctrine]] rather than a neutral spell resource.
- Binder form cannot make slavery, predatory ownership, or soulbinding morally valid.
- Bard practice obeys [[Harmonic Shellcraft]]; its healing reaches only recoverable Health and its compulsion remains accountable force.
- No player vocation creates a new plane, permanent Shell anchor, soul route, or universal metaphysical exception.

## Party and Field Role

The field party is the player plus up to three active companions. The protagonist remains the controlled character. Companions contribute combat behavior, field capability, conversation, judgement, investigation, and personal quests without replacing player agency.

Inactive companions follow their own routines, assignments, and personal campaigns unless a defined state removes them. They may act, receive support, compromise themselves, or depart without the player's permission, but cannot die unseen. [[Companion Constitution]] and [[Companion Autonomy and Act Convergence]] control those rules.

## Debt and Freedom

Debt is sustained pressure and a long-form player state, not a hidden timer. It may change through payment, service credit, fraud exposure, sponsorship, political settlement, defiance, or escape. Becoming free has material and relational consequences; remaining with the Blackjacks after freedom must be a meaningful choice.

Refusing one employer does not automatically end the campaign. It changes access, equipment, debt terms, faction standing, or danger applied to the player and their associates.

## Production Boundary

Exact curves, damage, frame data, input mappings, slot budgets, social probabilities, affliction rates, and final compatibility rules remain in [[Provisional Player Balance Appendix]]. Source-derived skill language remains author-only until cleared through [[Vocation Originalization Register]].

## Authority Boundaries

- Player systems doorway: [[Player Systems MOC]]
- Training: [[Vocation Training Network]]
- Legal treatment: [[Public Power and Vocation Law]]
- Companion relationships: [[Companion Constitution]]
- Companion campaign architecture: [[Companion Campaign Architecture]]
- Faction service: [[Factions Overview]]
- Seasonal availability: [[Season and World-State Constitution]]
- Identity recognition: [[Player Identity Reactivity Matrix]]
- Campaign use: [[Grenzburg Campaign Spine]]

## Navigation

- [[Grenzburg Game Constitution]]
- [[Player Systems MOC]]
- [[Blackjack Corps]]
- [[Crisis Charter of Secondment]]
- [[Grenzburg MOC]]
