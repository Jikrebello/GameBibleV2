---
type: index
status: archived
canon_level: obsolete
visibility: author-only
archive_reason: "Control snapshot before the Grenzburg RPG Experience Constitution pass"
archived_from: "02 Playable Settings/Grenzburg/10 Production Bible/20 Player Systems/Player Systems MOC.md"
archived_on: 2026-07-24
superseded_by: "[[Player Systems MOC]]"
era: ["Present Day"]
region: ["Grenzburg"]
culture: []
related_factions: ["Blackjack Corps", "Medori Bank", "Duchy of Grenzburg"]
appears_in: ["Grenzburg"]
needs: []
system_family: player
player_system: index
tags: [game-ready]
---
# Player Systems MOC

This is the doorway to Grenzburg's narrative player systems. [[Player Character Constitution]] controls the protagonist and the non-negotiable rules. The notes below explain what those rules mean for character creation, field play, progression, training, and the world.

Exact damage, frame data, input mappings, probability curves, slot budgets, and final numerical balance are not canon here. They remain in [[Provisional Player Balance Appendix]] until production testing.

## Authority Desk

| Question | Controlling note |
|---|---|
| Who is the player and what can change? | [[Player Character Constitution]] |
| What is chosen at character creation? | [[Character Creation and Identity]] |
| What do the statistics mean? | [[Player Statistics and Resolution]] |
| How do dialogue and knowledge checks work? | [[Social Skills and Knowledge]] |
| How does conduct shape available commitments? | [[Virtue and Sin Tendencies]] |
| What can every character physically do? | [[Shared Movement and Physical Interaction]] |
| Which non-combat abilities are trained? | [[Proficiencies and Field Skills]] |
| How do harm, exhaustion, conditions, and weight read? | [[Health Recovery Afflictions and Encumbrance]] |
| What may a vocation equip? | [[Equipment and Skill Loadouts]] |
| How do levels, mastery, and switching work? | [[Vocation Progression and Mastery]] |
| Who teaches each discipline? | [[Vocation Training Network]] |
| How does the law regard player powers? | [[Public Power and Vocation Law]] |

## Character Framework

- [[Character Creation and Identity]]
- [[Player Statistics and Resolution]]
- [[Social Skills and Knowledge]]
- [[Virtue and Sin Tendencies]]

## Shared Field Systems

- [[Shared Movement and Physical Interaction]]
- [[Proficiencies and Field Skills]]
- [[Health Recovery Afflictions and Encumbrance]]
- [[Equipment and Skill Loadouts]]
- [[Vocation Progression and Mastery]]

## Starting Vocations

| Vocation | Native kit | Primary field promise |
|---|---|---|
| [[Fighter]] | one-handed martial weapon, shield, heavy armour | hold the line, counter, stagger, protect |
| [[Slayer]] | paired blades, grappling knives, light armour | infiltrate, reposition, climb, exploit weak points |
| [[Hunter]] | longbow, crafted arrows, light field armour | read terrain and creatures, control range |
| [[Warlock]] | paired war-wands, robes or light armour | weave elemental scripts while managing Arcane Burn |
| [[Medium]] | medium's rod, shade roster, shadow fields | turn defeated forms into temporary tactical pressure |
| [[Rune Fist]] | inscribed gauntlets, medium armour | catch force and return it through close elemental counters |

## Advanced and Custom Vocations

- [[Paladin]] is earned through Bernard's third personal quest and completed through his fourth.
- [[Binder]] is earned through Zafir's third personal quest and completed through his fourth.
- [[Custom Vocation Workshop]] opens as a metagame system after two authored vocations are fully mastered.

## Production Controls

- [[Vocation Training Network]]
- [[Public Power and Vocation Law]]
- [[Provisional Player Balance Appendix]]
- [[Vocation Originalization Register]]

![[Player Systems.base]]

## Fixed Rules

1. Combat is third-person action combat.
2. The player equips one authored or custom vocation at a time.
3. Authored vocations retain their native equipment and skill language.
4. Global character level and vocation mastery are separate.
5. The active vocation supplies level-up growth.
6. Every vocation can complete every critical route.
7. Custom vocations are a metagame feature, not an in-world school.
8. Medium shades are non-conscious imprints, never captured souls.
9. White-Flame techniques require moral coherence.
10. Human souls and coerced persons cannot become responsible Binder assets.

## Source Boundary

The imported [[Game Lore Bible]] is archived source material. Its statistics, proficiencies, movement intentions, and vocation functions informed this system. Its later peoples, cosmology, creatures, companions, and world history did not enter active canon.

## Return

- [[Grenzburg MOC]]
- [[Grenzburg Game Constitution]]
- [[Current Focus Board]]
