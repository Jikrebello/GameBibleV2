---
type: reference
status: archived
canon_level: obsolete
visibility: author-only
era: ["Present Day"]
region: ["Grenzburg"]
culture: ["Eclessian", "Folk", "Chapel-Folk", "Grobi"]
related_factions: ["Duchy of Grenzburg", "Folk kindreds", "Grobi"]
appears_in: ["Grenzburg"]
needs:
  - assign remaining marquee, local, training, and job ownership and final encounter budgets
map_id: "GR-REGISTER"
worldspace_role: location-register
subregion: ""
district: ""
map_visibility: author-only
danger_band: mixed
travel_services: []
seasonal_states: [autumn, winter, spring, summer]
dungeon_scale: mixed
tags: [game-ready]
archive_reason: "Snapshot before romance and same-sex sworn-bond episodes were separated into distinct authored quests"
archived_from: "02 Playable Settings/Grenzburg/20 Region and Locations/Grenzburg Worldspace Location Register.md"
archived_on: 2026-07-25
superseded_by: "[[Grenzburg Worldspace Location Register]]"
---
# Grenzburg Worldspace Location Register

This register controls content placement below the regional-hub level. A name in this table reserves a physical site and broad purpose. [[Main Campaign MOC]] now assigns critical-path ownership; later waves assign remaining side content, room layouts, and implementation budgets.

## Allocation Summary

| Scale | Count | Use |
|---|---:|---|
| City district anchors | 40 | Five reusable anchors in each of eight districts; controlled by [[Grenzburg City Anchor Register]] |
| Cross-city anchors | 12 | Bridge, walls, underways, riverworks, roofs, and camps; controlled by [[Grenzburg City Anchor Register]] |
| Exterior major spaces | 9 | Regional signature spaces and campaign-scale sites |
| Exterior medium sites | 14 | Two authored quest or dungeon spaces per region |
| Exterior minor landmarks | 49 | Seven navigation, encounter, story, or job anchors per region |

Named dungeons retain a cleared, occupied, damaged, sealed, or transformed state. Only sites explicitly marked **renewable** may repopulate for hunts, camps, or jobs.

## Main-Campaign Claims

| Site family | Controlling chapter or operation |
|---|---|
| Broken Tollworks and Three Mills Reach | [[The Closing Roads]] and [[Roads Worth Saving]] |
| Tannbruck, Watch Hill, and Tann Crossing | MQ-04 through [[Tannbruck Falls]], then [[The Land Reclaimed]] |
| Emerald Drake Range and Southwood thresholds | [[The Emerald Trail]] and [[A Drake in Winter]] |
| Tuskway Redoubt and Herd Rest | [[Break the Southern Horn]] and the Tuskway anchor operation |
| Cold-Iron Deeps lower route | [[The Open Thaw]] |
| Old River Arsenal | River Arsenal anchor operation under [[The Land Reclaimed]] |
| Three Horn Depot | [[Gerhold's Choice]] |
| White Mud Causeway and Last Hearth | [[The Unending Muster Main Quest|The Unending Muster]] and [[The Dead March]] |
| Deep Muster Approaches and interior | [[The Deep Muster Main Quest|The Deep Muster]] |

Main ownership does not consume a whole site. Side content may reuse accessible layers when its timing and persistent state agree with the controlling chapter.

## Major Spaces

| ID | Region | Site | Scale | Initial map state | Reserved function |
|---|---|---|---|---|---|
| GR-1-D1 | [[Grenz Lowlands]] | [[Broken Tollworks]] | major | public | grounded civil ruin and early dungeon language |
| GR-2-D1 | [[Longlake Country]] | [[Sunken Causeway]] | major | restricted | flooded Atherian infrastructure and seasonal traversal |
| GR-3-D1 | [[Wolfsmere Woods]] | [[Bellless Hold]] | major | restricted | occupation ruin under place-law pressure |
| GR-4-D1 | [[Southwood Barrowlands]] | [[Emerald Drake Range]] | major | restricted | hunt territory and winter lair complex |
| GR-4-D2 | [[Southwood Barrowlands]] | [[Barrow of the First Chieftain]] | major | hidden | optional high-level Heart-Stone site; never critical path |
| GR-5-D1 | [[Highlake Uplands]] | [[Cold-Iron Deeps]] | major | restricted | vertical mine complex and industrial frontier |
| GR-6-D1 | [[Upper Grenz]] | [[Old River Arsenal]] | major | restricted | river-defense works and recoverable military industry |
| GR-6-H2 | [[Upper Grenz]] | [[Fort Tannbruck]] | evolving major | public | fixed Act I fall, winter front, and spring reclamation |
| GR-7-D1 | [[Frostfenn March]] | [[The Deep Muster]] | mega | hidden | final Qianglong death-and-war complex |

[[Deep Muster Approaches]] controls the final dungeon's surface basin and may be entered before its interior is available.

## Medium Sites

| ID | Region | Site | Form | Placement purpose |
|---|---|---|---|---|
| GR-1-M1 | Grenz Lowlands | Three Mills Reach | mill hamlet and shared leat | food, labor, flood control, and local rivalry |
| GR-1-M2 | Grenz Lowlands | Black Road Quarry | worked civil quarry | construction jobs, bandit cover, and ordinary danger |
| GR-2-M1 | Longlake Country | [[Bent-Oak Refuge]] | Chapel-Folk refuge house | shelter law, road service, child road-school, and winter triage |
| GR-2-M2 | Longlake Country | Netter's Isle | seasonal fishing island | ferry play, smuggling, and ice-state change |
| GR-3-M1 | Wolfsmere Woods | Grey-Withy Crossing | outer-wood ford and waystone | negotiated access and fixed remote travel |
| GR-3-M2 | Wolfsmere Woods | Drowned Roll Paths | flooded forest tracks | environmental navigation and lost memorial records |
| GR-4-M1 | Southwood Barrowlands | Ashfield Scar | war-magic burn zone | bounded Shell residue and recovery work |
| GR-4-M2 | Southwood Barrowlands | [[Seven Mounds]] | lesser barrow group | Elara quest two; coerced excavation, burial duty, rescue, and delver temptation |
| GR-5-M1 | Highlake Uplands | Ghost-Foot Stair | crag path and waystone | vertical traversal and Ghost-Foot access |
| GR-5-M2 | Highlake Uplands | Tuskway Redoubt | abandoned marcher fort | military road control and defensible return hub |
| GR-6-M1 | Upper Grenz | Fellwater Camps | lumber and rafting camps | fuel production, labor conflict, and river rescue |
| GR-6-M2 | Upper Grenz | Tann Crossing Farms | fortified farm belt | evacuation, supply, and spring resettlement |
| GR-7-M1 | Frostfenn March | Three Horn Depot | ruined migration store | Grobi routes, survival stores, and contested evidence |
| GR-7-M2 | Frostfenn March | White Mud Causeway | broken fen road | seasonal closure, dead-march route, and engineering |

Medium-site names are canon. [[Seven Mounds]] is claimed by [[A House Fed by Bones]]. Other casts, owners, and outcomes remain unassigned until a quest or faction wave claims them.

## Minor Landmarks

### Grenz Lowlands

| ID | Landmark | Use | Renewable |
|---|---|---|---|
| GR-1-L1 | Split Elm Inn | safe node, rumor exchange, carriage stop | no |
| GR-1-L2 | South Ferry | river crossing and service-state indicator | no |
| GR-1-L3 | Willow Graveyard | local history and residue tutorial space | no |
| GR-1-L4 | Long Field Shrine | road devotion and public consequence marker | no |
| GR-1-L5 | Floodpost Seven | flood measure and spring route clue | no |
| GR-1-L6 | Haymaker's Copse | hunt, gathering, and ambush ground | yes |
| GR-1-L7 | Old Coach Burn | wreck site and roadside story anchor | no |

### Longlake Country

| ID | Landmark | Use | Renewable |
|---|---|---|---|
| GR-2-L1 | Pikehook Marsh | reeds, predators, and search jobs | yes |
| GR-2-L2 | Cold Net Cove | boat landing and hidden approach | no |
| GR-2-L3 | Reedmile Chapel | sanctuary and Chapel-Folk service | no |
| GR-2-L4 | West Smokehouse | food supply and labor anchor | no |
| GR-2-L5 | Eel Bridge | road chokepoint and flood-state marker | no |
| GR-2-L6 | Ice Bell Rock | winter route warning and lake landmark | no |
| GR-2-L7 | Lake Road Cairn | navigation, memorial, and encounter anchor | yes |

### Wolfsmere Woods

| ID | Landmark | Use | Renewable |
|---|---|---|---|
| GR-3-L1 | Outer Road Stone | first permission boundary | no |
| GR-3-L2 | Silent Ford | stealth crossing and weather route | no |
| GR-3-L3 | Root-Witness Grove | place-law hearing and consequence site | no |
| GR-3-L4 | Nine-Breath Shelter | remote safe node with strict custom | no |
| GR-3-L5 | Cold-Hand Spring | healing resource and taboo | no |
| GR-3-L6 | Owl-Mark Blind | hunter observation and tracking jobs | yes |
| GR-3-L7 | Lakekeeper Boundary | sacred inner limit; no waystone beyond it | no |

### Southwood Barrowlands

| ID | Landmark | Use | Renewable |
|---|---|---|---|
| GR-4-L1 | Burnt Antler Camp | hunter staging and monster sign | yes |
| GR-4-L2 | Split Pine Ravine | climb route and winter hazard | no |
| GR-4-L3 | Widow's Clearing | local story and ambush ground | yes |
| GR-4-L4 | Charcoal Hollow | work camp and smoke-navigation anchor | no |
| GR-4-L5 | First Snow Cairn | seasonal warning and route fork | no |
| GR-4-L6 | Mireglass Pool | alchemy resource and natural hazard | yes |
| GR-4-L7 | Hunter's Last Mark | Drake-range threshold and retreat warning | no |

### Highlake Uplands

| ID | Landmark | Use | Renewable |
|---|---|---|---|
| GR-5-L1 | Needle Tarn | water source, weather mirror, and hunt site | yes |
| GR-5-L2 | Ropehouse Ruin | autumn shoring encounter; sealed survey loft exposed for [[The First Honest Find]] in summer | no |
| GR-5-L3 | Windcut Mine | minor extraction delve | no |
| GR-5-L4 | Raven Shelf | overlook and aerial-threat warning | yes |
| GR-5-L5 | Blue Ice Cave | seasonal cave and cold shelter | yes |
| GR-5-L6 | Scree Chapel | marcher devotion and avalanche refuge | no |
| GR-5-L7 | East Watch Cairn | Tuskway observation point | no |

### Elara Embedded Claims

- **Ropehouse Ruin (EL-E08):** its accessible outer works support an autumn exploration beat that can produce `elara_shoring_knowledge`. The deeper survey loft remains sealed until spring repairs expose it for [[The First Honest Find]].
- **Raven Shelf and Blue Ice Cave:** [[Raven Shelf at Dawn]] uses the shelf, old signal line, and cave refuge without removing their ordinary navigation and weather functions.
- **Finder's Share (EL-E07):** the later local-content wave must place this dispute at a living work or custody site. Its completed form can produce `elara_living_custodian_support`; it may not describe unoccupied ruin as the only claimant.

### Upper Grenz

| ID | Landmark | Use | Renewable |
|---|---|---|---|
| GR-6-L1 | Three-Log Bridge | local crossing and repair state | no |
| GR-6-L2 | Sawdust Chapel | labor devotion and casualty register | no |
| GR-6-L3 | Upper Ferry | river service and military movement | no |
| GR-6-L4 | Black Fir Watch | road warning post and patrol anchor | yes |
| GR-6-L5 | Charcoal Bend | fuel depot and river ambush point | yes |
| GR-6-L6 | Lost Rafter Camp | rescue and missing-worker jobs | yes |
| GR-6-L7 | River Teeth | rapids, wreckage, and spring hazard | yes |

### Frostfenn March

| ID | Landmark | Use | Renewable |
|---|---|---|---|
| GR-7-L1 | Fenn Lantern | last maintained road light | no |
| GR-7-L2 | Hollow Spruce Shelter | deep-south safe node | no |
| GR-7-L3 | Herd Rest | migration camp and negotiation ground | yes |
| GR-7-L4 | Last Road Marker | end of ordinary ducal survey | no |
| GR-7-L5 | Bone-Reed Pool | residue warning and fen hazard | yes |
| GR-7-L6 | Snowblind Rise | whiteout navigation landmark | yes |
| GR-7-L7 | Black-Ice Ford | dangerous seasonal crossing | yes |

## Bernard Embedded Care Routes

- **Pikehook snare line (BE-E07):** one authored exploration of Pikehook Marsh can identify the abandoned hunting line, the wounded bear's likely movement, and a safe child-withdrawal route. Recording and marking it sets `bernard_pikehook_snare_line_mapped`; ordinary renewable search jobs do not repeat the asset.
- **West Refuge underways (BE-E08):** one authored exploration from West Refuge Hall into the city underways can clear and mark a nonpublic patient route. Completing it without handing the route to an exclusive underworld claimant sets `bernard_underways_evacuation_route`.

## Content Placement Rules

1. A critical-path chapter may claim a reserved site, but cannot silently move it to another region.
2. A faction or companion quest records its site's seasonal window and persistent result before blocking begins.
3. Minor landmarks receive dedicated notes only when their quest, named cast, or state complexity justifies one.
4. Renewable sites reset only their designated hunt, camp, or job layer. Named people, discoveries, structures, and moral outcomes persist.
5. New sites require a map ID and must preserve each region's traversal identity and content budget.

## Navigation

- [[Grenzburg Regional Geography]]
- [[Grenzburg Exterior Regions Overview]]
- [[Grenzburg City Anchor Register]]
- [[Grenzburg Seasonal Worldspace Matrix]]
- [[Grenzburg MOC]]
